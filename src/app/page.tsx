...
export default function BookingPortal() {
  ...
  useEffect(() => {
    if (user) {
      fetchBookings();
      fetchStaffList(); // Loaded for both staff and admin now to fill selectors
    }
  ...
  const addonsTotal = selectedAddons.reduce((sum, addonId) => {
    const addonPrice = prices.addons[addonId] ?? 0;
    return sum + (addonPrice * (parseInt(pax) || 1));
  }, 0);
      ...
      {!user ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          <form onSubmit={handleSubmitBooking} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ADDONS.map((addon) => {
                    ...
                    const addonPrice = prices.addons[addon.id] ?? 0;
                    return (
                      <button
                        key={addon.id}
                        type="button"
                      ...
                      >
                        ...
                        <span className="text-[8px] opacity-60">+₹{addonPrice}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {/* Section 2.5: Staff Assignment for Guest and Assist */}
            <div className="glass-panel p-4 rounded-2xl flex flex-col gap-4">
              <div className="flex items-center gap-2 pb-2 border-b border-zinc-800">
            ...
            <div className="glass-panel p-4 rounded-2xl flex flex-col gap-4">
              ...
              <div className="bg-sky-500/10 border border-sky-500/25 rounded-xl p-3 flex justify-between items-center">
                <div className="flex flex-col">
                  ...
                  <span className="text-[8px] text-zinc-500">Rate + (Add-ons * Pax)</span>
                </div>
